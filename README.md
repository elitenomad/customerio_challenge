...
## Design decsions and limitations

- Added `EventIds` attribute to handle duplicate event counts per UserID and Event Name.
- Added a flag to the `main.go` so the input can be driven by command line
  - for e.g to start the program,`go run main.go --input="data/messages.1.data"`
- Updated `verify/main.go` code to ignore the additional attributes that were added in comparison methods.
- Implemented given `datastore` interface to power the REST API functions.
